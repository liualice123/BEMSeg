...
class CLIPResNetWithAttention(nn.Module):
    ...
    def __init__(self, layers, output_dim=1024, input_resolution=224, width=64, pretrained=None, **kwargs):
        ...
        # self.attnpool_1 = AttentionPool2d(input_resolution // 16, embed_dim, 32, output_dim)
    ...
    def init_weights(self, pretrained=None):
        ...
        if isinstance(pretrained, str):
            checkpoint = torch.jit.load(pretrained, map_location='cpu').float().state_dict()
            # print("test1111")
            # print(checkpoint.keys())   # the length of checkpoint is 492  visual.conv1.weight

            state_dict = {}
            # new = torch.FloatTensor(32,9,3,3)
            # new = new.permute(1,0,2,3)
            # new = new.normal_(mean=0, std=0.01)

            for k in checkpoint.keys():
                if k.startswith('visual.'):
                    new_k = k.replace('visual.', '')
                    state_dict[new_k] = checkpoint[k]
                    # layer_conv1 = state_dict['conv1.weight']
                    # print("test2222")
                    # layer_conv1 = layer_conv1.permute(1,0,2,3)
                    # print(len(layer_conv1))     # length is 32
                    # nst = enumerate(layer_conv1)
                    # _, batch = next(nst)
                    # print(len(batch))   # length is 3
                    # print(len(batch[0]))
                    # print(len(batch[0][0]))

                    # for i, output_channel in enumerate(layer_conv1):
                        # print(i)
                        # new[i] = output_channel[i]
                    # print(batch.shape())
                    # state_dict['conv1.weight'] = new.permute(1,0,2,3)
    ...
    def forward(self, x):
        ...
        x = self.layer3(x)
        outs.append(x)
        # x1_global, x1_local = self.attnpool_1(x)
        x = self.layer4(x)
        outs.append(x)

        x_global, x_local = self.attnpool(x)
        outs.append([x_global, x_local])
        # outs.append([x1_global, x1_local])
